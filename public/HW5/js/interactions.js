$(document).ready(function(){
  ...
  $("#edit-modal form").submit(function(e) {
    ...
    if (oldName != values["name"]) {
      itemExists(type,values["name"]).then(function(exists) {
        if (exists) displayEditError(values["name"]);
          ...
          removeItem(oldName, type).then(function() {
            if (type == "Breweries") {
              addBrewery(values["name"], values["name"], values["location"]).then(function() { location.reload(true); });
            }
            else {
              addBeer(values["name"], values["brewery"], values["name"], values["type"], values["style"]).then(function() { location.reload(true); });
            }
          });
        }
      });
      ...
      if (type == "Breweries") {
        editBrewery(values["name"], values["name"], values["location"]).then(function() { location.reload(true); });
      }
      else {
        editBeer(values["name"], values["brewery"], values["name"], values["type"], values["style"]).then(function() { location.reload(true); });
      }
    }
  });
  ...
  $(document.body).on('click', '#card-content li a', function(e){
    ...
    if(currentPage == COMMUNITY_PAGE &&
      $(this).attr("id") != "addnew" &&
      $(this).parent().attr("class") != "delete-confirm") {
  ...
  //after deletion, check favs to remove removed items
  function clearRemovedFav(selected, type) {
    var item = firebase.database().ref('Users/');

    item.once("value", function(snapshot) {
      if (snapshot.val() !== null) {
        for( var each in snapshot.val() ) {
          removeFav(selected, each);
        }
      }
    }, function (error) {
      console.log("Error: " + error.code);
  ...
  function getProfilePic() {
    var userId = firebase.auth().currentUser.uid;
    var user = firebase.database().ref('Users/'+userId+'/Profile/picture');

    user.once("value", function(snapshot) {
      if (snapshot.val() !== null) {
        $("img#profile").attr("src", snapshot.val());
      }
    }, function (error) {
      console.log("Error: " + error.code);
    });
  ...
  function prepareHeaders() {
    ...
    if (data === null) {
      ...
      if (category == "brewery") {
        if(currentPage == BREWERY_PAGE) $("h1").text(decodeURIComponent(data["brewery"])+" Beers");
      }
  ...
  firebase.auth().onAuthStateChanged(firebaseUser => {
    ...
    //if user is authenticated, dynamically load data
      ...
      if (data === null) viewAll();
        ...
        for(var category in data) break;
        if (category == "brewery")
          viewBrewery(decodeURIComponent(data["brewery"]));
        else if (category == "fav")
          viewFavs(decodeURIComponent(data["fav"]));
      }
      getProfilePic();
    }
  });
